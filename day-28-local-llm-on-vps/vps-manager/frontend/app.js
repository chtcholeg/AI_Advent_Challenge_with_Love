...
/* ── Scenarios ───────────────────────────────────────────────────────────────── */
const EMOJI = { shield: '🛡️', globe: '🌐', server: '⚙️', scale: '⚖️', cpu: '🤖' };
...
function dm_runOp(type) {
  ...
  dm_ws.onopen = () => {
    ...
    if (type === "open_ports") {
      payload.port_web     = dm$("dm-port-web").value;
      payload.port_openvpn = dm$("dm-port-openvpn").value;
      payload.port_custom  = dm$("dm-port-custom").value.trim();
    }
...
/* ── Ports panel ─────────────────────────────────────────────────────────────── */
function dm_togglePortsPanel() {
  dm$("dm-card-ports").classList.toggle("hidden");
}

function dm_hidePortsPanel() {
  dm$("dm-card-ports").classList.add("hidden");
}

function dm_applyPorts() {
  dm_hidePortsPanel();
  dm_runOp("open_ports");
}
...
function dm_setActionBtnsDisabled(disabled) {
  ["dm-btn-deploy", "dm-btn-restart", "dm-btn-status", "dm-btn-logs", "dm-btn-install", "dm-btn-ports"].forEach(id => {
