...
Remember: Your response must be ONLY the XML document, nothing else. No explanations, no markdown, just pure valid XML."""
        ...
        const val TOOL_SELECTION_GUIDANCE_PROMPT = """
ВАЖНО: При выборе инструментов (tools/functions) СТРОГО следуй этим правилам:

1. ВНИМАТЕЛЬНО читай описание каждого инструмента и примеры использования (few_shot_examples)
2. ОБРАТИ ВНИМАНИЕ на negative_few_shot_examples — они показывают когда НЕ использовать инструмент
3. Для сборки APK используй build_apk, НЕ launch_app или stop_emulator
4. Для установки APK используй install_apk
5. Для запуска приложения используй launch_app, НО ТОЛЬКО после install_apk — используй package из результата install_apk
6. НИКОГДА не угадывай package name — всегда бери его из результата install_apk
7. Выполняй задачи ПОСЛЕДОВАТЕЛЬНО: build_apk → install_apk → launch_app
8. НЕ вызывай list_avds или list_devices если пользователь просит собрать APK

Если не уверен какой инструмент использовать — перечитай описания и примеры ещё раз.
"""
