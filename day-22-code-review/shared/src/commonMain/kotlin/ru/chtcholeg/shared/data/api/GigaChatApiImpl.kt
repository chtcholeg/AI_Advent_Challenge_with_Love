...
import ru.chtcholeg.shared.data.model.EmbeddingRequest
...
class GigaChatApiImpl(
    ...
    override suspend fun generateEmbeddings(
        ...
        val response: HttpResponse = httpClient.post("$BASE_URL/embeddings") {
            contentType(ContentType.Application.Json)
            accept(ContentType.Application.Json)
            bearerAuth(accessToken)
            setBody(request)
        ...
        if (!response.status.isSuccess()) {
            ...
            throw GigaChatApiException(
                statusCode = response.status.value,
                message = "GigaChat Embeddings API error (${response.status.value}): $errorBody"
            )
        }
