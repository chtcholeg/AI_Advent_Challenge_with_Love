# GigaChat Multiplatform Chat Application (Day 21 - Developer Assistant)
...
## Updates in Day 21: Developer Assistant with Git MCP

This version adds a **Git MCP Server** - enabling AI Agent to work with Git repositories as a developer assistant. The agent can check status, view history, create commits, manage branches, and more.

### New in Day 21

**Git MCP Server** - Python-based MCP server providing Git operations:
- 📊 Read operations: status, log, diff, branches, blame
- ✏️ Write operations: add, commit, checkout, pull, push
- 🚀 Fast setup: 5-minute quick start with `./START.sh`
- 🔌 Easy integration with AI Agent via MCP protocol

See [mcp-servers/README.md](mcp-servers/README.md) for full documentation.
...
- **MCP Composition** (Days 11-15): 7 MCP servers with Docker integration
...
Day 20 focused on the RAG Chat capabilities built on Days 16-19.

### Day 19: Citation and Sources (Clickable Web URLs)

Introduced **Clickable Web Sources** - click on source references in AI Agent to open web pages directly in browser. FileOpener automatically detects URLs (http/https) and opens them in browser, with cross-platform support (Desktop + Android). Sources displayed as blue, underlined, bold text.

New indexing commands: `indexUrl` and `indexUrls` for web page indexing.
...
**Status**: Fully functional - MVI architecture, MCP integration, RAG support, command system
...
├── DAY_21_SUMMARY.md            # Day 21 summary
└── GIT_MCP_SETUP.md             # Git MCP Server setup guide
```
...
```bash
cd /Users/shchepilov/AndroidStudioProjects/AI_Advent_Challenge_with_Love/day-21-developer-assistant
```
...
Servers will start on ports 8000-8006. For Git MCP Server (port 8010), see below. Add them in app Settings → MCP Servers:
...
The Git MCP Server runs independently from the launcher on port 8010:
...
Add in AI Agent: Settings → MCP Servers → Add → URL: `http://localhost:8010/sse`

See [GIT_MCP_SETUP.md](GIT_MCP_SETUP.md) for details.

### Server Documentation

MCP servers are located in a separate project directory. Each server includes documentation with usage examples, tool descriptions, configuration options, and API endpoint details.

**Available Servers**: GitHub (8000), Telegram (8001), Weather (8002), TimeService (8003), Currency (8004), FileOps (8005), Docker (8006), Git (8010)
...
- **DAY_21_SUMMARY.md** - Day 21 Developer Assistant summary
- **GIT_MCP_SETUP.md** - Git MCP Server quick setup
...
- **ai-agent/GIT_README_DETECTION.md** - Git-based README detection strategies

**Commands System:**
- **COMMANDS_README.md** - User guide for commands
- **COMMANDS_GUIDE.md** - Technical documentation for adding commands
