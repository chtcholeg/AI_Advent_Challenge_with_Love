...
Назначен на: {ticket.get('assignedTo', 'Не назначен')}"""
        ...
        tickets = load_tickets()
        limit = min(int(arguments.get("limit", 20)), 100)  # Default 20, max 100
        ...
        if not tickets:
            ...
            logger.info(f"[list_tickets] Found {len(tickets)} tickets, showing {min(limit, len(tickets))}")
            result = f"Найдено тикетов: {len(tickets)} (показано: {min(limit, len(tickets))})\n\n"
            for ticket in tickets[:limit]:
                result += f"- {ticket['id']}: {ticket['subject']} [{ticket['status']}] ({ticket['priority']} priority)\n"

            if len(tickets) > limit:
                result += f"\n... и ещё {len(tickets) - limit} тикетов (используйте параметр limit для увеличения)\n"
...
Обновлен: {ticket['updated']}"""
        ...
        limit = min(int(arguments.get("limit", 10)), 50)  # Default 10, max 50

        logger.info(f"[search_tickets] Searching with query: '{query}', limit: {limit}")
        ...
        result += f"(Отсортировано по релевантности, показано топ-{min(limit, len(search_results))})\n\n"

        for i, search_result in enumerate(search_results[:limit], 1):
        ...
        if len(search_results) > limit:
            result += f"... и ещё {len(search_results) - limit} тикетов (используйте параметр limit для увеличения)\n"
        ...
        user_id = arguments["user_id"]
        limit = min(int(arguments.get("limit", 20)), 100)  # Default 20, max 100
        logger.info(f"[get_user_tickets] Getting tickets for user: {user_id}, limit: {limit}")
...
Тикеты пользователя ({len(user_tickets)}, показано: {min(limit, len(user_tickets))}):
"""

        if not user_tickets:
            result += "Нет тикетов"
        else:
            for ticket in user_tickets[:limit]:
                result += f"""
...
"""
            ...
            if len(user_tickets) > limit:
                result += f"\n... и ещё {len(user_tickets) - limit} тикетов (используйте параметр limit для увеличения)\n"
