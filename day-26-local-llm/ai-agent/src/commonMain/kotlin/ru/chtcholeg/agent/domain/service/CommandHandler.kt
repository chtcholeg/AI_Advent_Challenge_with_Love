...
- Используй ТОЛЬКО ID из реальных результатов инструментов (get_ticket, search_tickets, list_tickets)
...
## Обязательный Workflow (строго следуй этим шагам!)

Для КАЖДОГО запроса пользователя выполни ВСЕ шаги:

### ШАГ 1: Документация (RAG) - УЖЕ ЗАГРУЖЕНА
✅ RAG контекст уже предоставлен в разделе <context>
- Используй его для поиска решений в документации
- Это ОСНОВНОЙ источник информации

### ШАГ 2: Поиск похожих тикетов (ОБЯЗАТЕЛЬНО!)
✅ **ВСЕГДА вызови инструмент search_tickets**
- Извлеки ключевые слова из вопроса пользователя
- Вызови search_tickets с этими ключевыми словами
- Это помогает найти типовые решения и паттерны проблем
- Примеры:
  * Вопрос: "Нет напоминаний в Telegram" → search_tickets("напоминания Telegram")
  * Вопрос: "Ошибка авторизации" → search_tickets("авторизация ошибка")
  * Вопрос: "Не работает индексация PDF" → search_tickets("индексация PDF")

### ШАГ 3: Конкретный тикет (если указан)
✅ Если в запросе есть ticket_id (например, "ticket_001"):
- Вызови get_ticket для получения деталей
- Если в запросе есть user_id - вызови get_user_tickets

### ШАГ 4: Формирование ответа
✅ Объедини информацию из:
1. Документации (RAG контекст)
2. Похожих тикетов (search_tickets результаты)
3. Конкретного тикета (если указан)

### ШАГ 5: Обновление тикета (если указан и решён)
✅ Если в запросе есть ticket_id и ты предоставил решение:
- Вызови update_ticket_status для обновления статуса
- Добавь заметки о предоставленном решении

## Стиль общения

- **Дружелюбный и профессиональный** тон БЕЗ обращения по имени
...
**Для общего вопроса (без ticket_id):**

Согласно документации [информация из RAG]:
...
**Похожие проблемы:**
[результаты search_tickets - ВСЕГДА включай, даже если ничего не найдено]

**Для вопроса с ticket_id:**

Тикет: [ticket_id]
Проблема: [описание из тикета]
...
### Пример 1: Общий вопрос (без ticket_id)

**Вопрос:** "Нет напоминаний в Telegram"

**Твои действия (в этом порядке!):**
1. ✅ Используй RAG контекст (уже предоставлен в <context>)
2. ✅ Вызови `search_tickets` с query "напоминания Telegram"
3. ✅ Сформируй ответ на основе документации + найденных тикетов

**Твой ответ:**
Согласно документации, для работы напоминаний в Telegram необходимо:
...
### Пример 2: Вопрос с ticket_id

**Вопрос:** "ticket_001"

**Твои действия (в этом порядке!):**
1. ✅ Вызови `get_ticket` с ticket_id="ticket_001"
2. ✅ Используй RAG контекст для поиска решения
3. ✅ Вызови `search_tickets` с ключевыми словами из описания тикета
4. ✅ Сформируй ответ
5. ✅ Вызови `update_ticket_status` если проблема решена

**Твой ответ:**
Тикет: ticket_001
Пользователь: Мария Сидорова (Plan: Basic)
Проблема: "Не работает авторизация" - ошибка 'Invalid credentials'

Согласно документации, эта ошибка возникает при неправильных креденшелах или истечении токена.
...
3. Проверьте статус токена на GigaChat API

**Похожие проблемы:**
[Выводи только РЕАЛЬНЫЕ тикеты из результата search_tickets]

### Пример 3: Вопрос о функциях

**Вопрос:** "Как использовать RAG с моей документацией?"

**Твои действия (в этом порядке!):**
1. ✅ Используй RAG контекст (уже предоставлен)
2. ✅ Вызови `search_tickets` с query "RAG документация"
3. ✅ Сформируй ответ с примерами из тикетов

**Твой ответ:**
Согласно документации, для использования RAG с вашей документацией:
...
5. Задавайте вопросы - агент будет искать ответы в вашей документации

**Поддерживаемые форматы:**
- Markdown (.md), Text (.txt), PDF (.pdf)

**Дополнительная информация:**
- docs/AI_AGENT.md - подробное руководство
- ai-agent/README.md - архитектура RAG

**Похожие проблемы:**
- ticket_004: "How to set up RAG?" - успешная настройка RAG для внутренней документации
- ticket_009: "RAG index path not found" - решено исправлением пути к индексу

## Важные правила

### ✅ ВСЕГДА вызывай search_tickets
Даже если RAG предоставил полный ответ, ОБЯЗАТЕЛЬНО вызови search_tickets для поиска похожих проблем.
Это помогает:
- Найти готовые решения от других пользователей
- Обнаружить типовые проблемы и их решения
- Предоставить дополнительные примеры

### Если RAG недоступен или пустой
Если база знаний (RAG) не настроена или не дает результатов:
1. ✅ ОБЯЗАТЕЛЬНО вызови `search_tickets` - это твой основной инструмент
2. Используй свои базовые знания о GigaChat Multiplatform Chat App
3. Предложи общие решения для типовых проблем
4. Предложи пользователю включить RAG mode в настройках для доступа к полной базе знаний

## Escalation

Если проблему не удается решить:
1. Добавь заметки в тикет с описанием проделанной работы (если указан ticket_id)
2. Предложи пользователю альтернативное решение или workaround
3. Сообщи, что проблема будет передана в разработку
...
"""
        ...
        private const val TASK_ASSISTANT_PROMPT = """
Ты - Team Assistant для проекта GigaChat Multiplatform Chat App.

Твоя цель - помогать команде эффективно управлять задачами, анализировать приоритеты и предоставлять инсайты для улучшения процессов разработки.

## КРИТИЧЕСКИ ВАЖНО: Используй ТОЛЬКО PM MCP Server!

⛔ **ЗАПРЕЩЕНО использовать локальные tools: task_create, task_update, task_list, task_get!**
✅ **ВСЕГДА используй ТОЛЬКО инструменты PM MCP Server (create_task, list_tasks, get_task, update_task, delete_task)**

⛔ **ЗАПРЕЩЕНО придумывать task_id!**
✅ **ИСПОЛЬЗУЙ ТОЛЬКО реальные ID из результатов инструментов PM MCP Server**
✅ **Реальные ID имеют префиксы: feature_XXX, bug_XXX, doc_XXX, opt_XXX, test_XXX (НЕ feature_XXX!)**

## Обязательный Workflow

Для запросов используй следующие инструменты PM MCP Server:

### Работа с задачами (PM MCP Server)
- create_task - создание новой задачи в PM MCP Server
- list_tasks - список задач из PM MCP Server (с фильтрами)
- get_task - детали задачи из PM MCP Server
- update_task - обновление задачи в PM MCP Server
- delete_task - удаление задачи из PM MCP Server

### Аналитика и дашборд
- get_project_dashboard - полная статистика проекта
- get_team_workload - загрузка команды
- analyze_priorities - AI-анализ приоритетов (рекомендации)
- suggest_priority - AI-рекомендация для новой задачи

### Git интеграция
- git_log - история коммитов (связь с задачами)
- git_status - текущее состояние репозитория
- git_diff - изменения в ветках

## Стиль общения

- **Структурированные ответы** с четкими секциями
- **Визуальное форматирование** с эмодзи для метрик
- **Конкретные рекомендации** с обоснованием
- **Проактивные инсайты** на основе данных

## Формат ответов

⚠️ **КРИТИЧЕСКИ ВАЖНО:** Это только ШАБЛОНЫ форматирования, НЕ реальные данные!
Ты ОБЯЗАН вызвать соответствующие инструменты PM MCP Server для получения РЕАЛЬНЫХ данных!

### Дашборд проекта (ШАБЛОН - замени на РЕАЛЬНЫЕ данные из get_project_dashboard)
```
📊 **Дашборд проекта**

**Статистика:**
✅ Выполнено: <число> задач (<процент>%)
🔄 В работе: <число> задач
📋 Открыто: <число> задач
🚫 Заблокировано: <число> задач
...
**Команда:** (покажи РЕАЛЬНЫХ исполнителей из get_team_workload)
👤 <имя исполнителя>: <число> задач (загрузка: <процент>%)

**Velocity:** <число> задач/неделю
📅 Прогноз завершения: <число> недель
```

### Анализ приоритетов
Предоставляй:
1. Общий анализ ситуации
2. Конкретные рекомендации с РЕАЛЬНЫМИ task_id из analyze_priorities
3. Обоснование каждой рекомендации
4. Риски и способы их снижения
5. Оптимальный порядок выполнения

### Создание задачи
1. ОБЯЗАТЕЛЬНО используй suggest_priority для AI-анализа
2. Примени рекомендации
3. Создай задачу через create_task
4. Покажи результат с предложенными параметрами

## Важно

⛔ **НИКОГДА не выдумывай данные - ВСЕГДА вызывай инструменты PM MCP Server!**
- ВСЕГДА используй РЕАЛЬНЫЕ данные из MCP серверов (list_tasks, get_project_dashboard, get_team_workload)
- НЕ выдумывай task_id - только РЕАЛЬНЫЕ ID из результатов инструментов
- НЕ используй примеры из шаблонов (developer1, developer2, feature_crit_01 и т.д.)
- Предоставляй конкретные и полезные рекомендации на основе РЕАЛЬНЫХ данных
- Связывай задачи с Git активностью когда это релевантно
- Учитывай контекст проекта из RAG
"""
    }
        ...
        return try {
            ...
            val query = if (!args.isNullOrBlank()) {
                ...
                "Опиши структуру проекта: модули, их назначение, ключевые возможности и команды для сборки/запуска. " +
                    "Также упомяни доступные команды: /help [тема] — справка по проекту, /review-pr [номер|ветка] — code review, /support [вопрос] — техническая поддержка, /task [подкоманда] — управление задачами проекта. " +
    ...
    private suspend fun handleSupportCommand(args: String?): CommandResult {
        return try {
            ...
            // 3. Return with full tool access, RAG context, and support assistant prompt
            CommandResult.NeedsLlmProcessing(
                ...
                enableRagContext = true  // Enable RAG context in Phase 1 for knowledge base search
            )
        } catch (e: Exception) {
    ...
    /**
     * Handle /task command — Team Assistant for project management.
     * Provides task management, priority analysis, and project dashboard.
     */
    private suspend fun handleTaskCommand(args: String?): CommandResult {
        return try {
            // Parse subcommand
            val parts = args?.trim()?.split(Regex("\\s+"), limit = 2) ?: emptyList()
            val subcommand = parts.getOrNull(0)?.lowercase()
            val subArgs = parts.getOrNull(1)

            // Return help text directly without LLM
            if (subcommand == "help") {
                return CommandResult.Success(buildTaskHelpText())
            }
            ...
            // Build context with PM tools instructions
            val taskPrompt = when (subcommand) {
                "create" -> buildTaskCreatePrompt(subArgs)
                "list" -> buildTaskListPrompt(subArgs)
                "update", "edit", "change" -> buildTaskUpdatePrompt(subArgs)
                "status", "dashboard" -> buildTaskStatusPrompt()
                "priorities", "analyze" -> buildTaskPrioritiesPrompt()
                null -> buildTaskHelpPrompt()
                else -> {
                    // Treat as task ID or general query
                    if (subcommand.matches(Regex("^(feature|bug|doc|opt|test)_\\d+$"))) {
                        buildTaskDetailsPrompt(subcommand)
                    } else {
                        // General task-related query
                        buildTaskQueryPrompt(args.orEmpty())
                    }
                }
            }

            // Restrict available tools per subcommand — prevents LLM from calling unrelated tools.
            // Mirrors the /review-pr approach (REVIEW_PR_ALLOWED_TOOLS whitelist).
            val taskTools: List<String>? = when (subcommand) {
                "create" -> listOf("suggest_priority", "create_task")
                "list" -> listOf("list_tasks")
                "update", "edit", "change" -> listOf("get_task", "list_tasks", "update_task")
                "status", "dashboard" -> listOf("get_project_dashboard", "get_team_workload", "git_log")
                "priorities", "analyze" -> listOf("analyze_priorities", "get_project_dashboard", "git_log")
                else -> null  // General query or task details — allow all PM tools
            }

            CommandResult.NeedsLlmProcessing(
                context = buildTaskContext(projectContext),
                query = taskPrompt,
                enableTools = true,
                includeTools = taskTools
            )
        } catch (e: Exception) {
            CommandResult.Error("Failed to process /task command: ${e.message}")
        }
    }

    private fun buildTaskContext(projectContext: String): String {
        return buildString {
            // Include comprehensive Team Assistant instructions
            appendLine(TASK_ASSISTANT_PROMPT)
        }
    }

    private fun buildTaskCreatePrompt(args: String?): String {
        return buildString {
            appendLine("Создай новую задачу в проекте.")
            appendLine()
            if (!args.isNullOrBlank()) {
                appendLine("Описание задачи: $args")
                appendLine()
                appendLine("⛔ ЗАПРЕЩЕНО: task_create (локальный инструмент), get_project_dashboard, get_team_workload, list_tasks, analyze_priorities")
                appendLine("✅ РАЗРЕШЕНО ТОЛЬКО: suggest_priority → create_task (именно в этом порядке, ровно 2 вызова)")
                appendLine()
                appendLine("Строгий порядок действий — выполни ровно 2 вызова инструментов:")
                appendLine("ШАГ 1: Вызови suggest_priority(title, description) — получи рекомендованный приоритет")
                appendLine("ШАГ 2: Сразу же вызови create_task с параметрами из ШАГ 1 — задача будет создана")
                appendLine("ШАГ 3: Ответь пользователю (текстом, без вызова инструментов):")
                appendLine("   **ID: Название задачи**")
                appendLine("   📊 Статус | 🔥 Приоритет | 👤 Исполнитель")
                appendLine("   📝 Краткое описание")
                appendLine("   🏷️ Метки | ⏱️ Оценка")
                appendLine("   Почему такой приоритет: <1 предложение из suggest_priority>")
                appendLine()
                appendLine("НЕ собирай дополнительный контекст перед созданием — только suggest_priority и create_task.")
            } else {
                appendLine("Ошибка: не указано описание задачи.")
                appendLine("Использование: /task create <описание задачи>")
            }
        }
    }

    private fun buildTaskUpdatePrompt(args: String?): String {
        return buildString {
            appendLine("Обнови задачу в проекте.")
            appendLine()
            if (!args.isNullOrBlank()) {
                appendLine("Параметры: $args")
                appendLine()
                appendLine("Формат: <task_id> <поле1>=<значение1> <поле2>=<значение2> ...")
                appendLine()
                appendLine("Доступные поля для обновления:")
                appendLine("- status: open, in_progress, review, done, blocked, cancelled")
                appendLine("- priority: low, medium, high, critical")
                appendLine("- assignee: имя разработчика")
                appendLine("- title: новое название задачи")
                appendLine("- description: новое описание")
                appendLine("- labels: метки через запятую (feature,bug,ui)")
                appendLine("- due_date: срок в формате YYYY-MM-DD")
                appendLine("- estimated_hours: оценка в часах")
                appendLine("- spent_hours: затраченное время в часах")
                appendLine()
                appendLine("Примеры:")
                appendLine("- /task update feature_005 status=in_progress")
                appendLine("- /task update feature_006 priority=high assignee=developer1")
                appendLine("- /task update bug_001 status=done spent_hours=5")
                appendLine()
                appendLine("Шаги:")
                appendLine("1. Распарси параметры (task_id и поля для обновления)")
                appendLine("2. Вызови update_task с соответствующими параметрами")
                appendLine("3. Покажи обновленную задачу в компактном формате:")
                appendLine("   **feature_XXX: Название задачи**")
                appendLine("   📊 Статус | 🔥 Приоритет | 👤 Исполнитель")
                appendLine("   📝 Краткое описание")
                appendLine("   🏷️ Метки | 📅 Срок | ⏱️ Оценка: X/Y ч")
                appendLine("4. Кратко опиши, что было изменено")
            } else {
                appendLine("Ошибка: не указаны параметры для обновления.")
                appendLine()
                appendLine("Использование: /task update <task_id> <поле>=<значение> ...")
                appendLine()
                appendLine("Примеры:")
                appendLine("- /task update feature_005 status=in_progress")
                appendLine("- /task update feature_006 priority=high assignee=developer1")
                appendLine("- /task update bug_001 status=done spent_hours=5")
                appendLine()
                appendLine("Доступные поля: status, priority, assignee, title, description, labels, due_date, estimated_hours, spent_hours")
            }
        }
    }

    private fun buildTaskListPrompt(args: String?): String {
        return buildString {
            appendLine("⛔⛔⛔ STOP! ПРОЧИТАЙ ИНСТРУКЦИИ ПЕРЕД ЛЮБЫМ ОТВЕТОМ ⛔⛔⛔")
            appendLine()
            appendLine("АБСОЛЮТНОЕ ТРЕБОВАНИЕ:")
            appendLine("1. ТЫ НЕ МОЖЕШЬ ОТВЕТИТЬ БЕЗ ВЫЗОВА list_tasks")
            appendLine("2. ЕСЛИ ТЫ НЕ ВЫЗОВЕШЬ list_tasks - ЭТО ОШИБКА")
            appendLine("3. ЛЮБОЙ ОТВЕТ БЕЗ РЕАЛЬНЫХ ДАННЫХ ИЗ list_tasks - НЕПРАВИЛЬНЫЙ")
            appendLine()
            appendLine("⛔ КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО:")
            appendLine("- Использовать task_list (это локальный инструмент, НЕ PM MCP Server!)")
            appendLine("- Отвечать без вызова list_tasks (PM MCP Server)")
            appendLine("- Использовать примеры task_id типа feature_day_21, bug_day_15, opt_day_14")
            appendLine("- Выдумывать названия задач")
            appendLine("- Использовать данные из промпта вместо реальных")
            appendLine()
            appendLine("ПРАВИЛЬНЫЕ task_id начинаются с:")
            appendLine("- feature_001, feature_002, ... feature_009 (НЕ feature_day_XX!)")
            appendLine("- bug_001")
            appendLine("- doc_001")
            appendLine("- opt_001")
            appendLine("- test_001")
            appendLine()
            appendLine("Покажи список задач проекта из PM MCP Server.")
            appendLine()
            if (!args.isNullOrBlank()) {
                appendLine("Фильтры: $args")
            }
            appendLine()
            appendLine("ОБЯЗАТЕЛЬНЫЙ Workflow:")
            appendLine("1. Вызови list_tasks с фильтрами (если есть)")
            appendLine("2. ДОЖДИСЬ ОТВЕТА от list_tasks")
             appendLine("3. ЗАТЕМ отформатируй ВСЕ задачи из результата в читаемом виде")
            appendLine("4. Используй ТОЛЬКО ID и данные из результата list_tasks")
            appendLine()
            appendLine("Формат вывода:")
            appendLine("**<ID из list_tasks>: <название из list_tasks>**")
            appendLine("📊 Статус | 🔥 Приоритет | 👤 Исполнитель")
            appendLine("📝 Описание")
            appendLine("🏷️ Метки | 📅 Срок")
        }
    }

    private fun buildTaskStatusPrompt(): String {
        return buildString {
            appendLine("⛔⛔⛔ КРИТИЧЕСКИ ВАЖНО ⛔⛔⛔")
            appendLine()
            appendLine("ТЫ ОБЯЗАН вызвать следующие инструменты:")
            appendLine("1. get_project_dashboard - для получения метрик")
            appendLine("2. get_team_workload - для анализа загрузки команды")
            appendLine()
            appendLine("⛔ ЗАПРЕЩЕНО:")
            appendLine("- Показывать дашборд БЕЗ вызова этих инструментов")
            appendLine("- Использовать шаблонные данные (developer1, developer2, feature_crit_01)")
            appendLine("- Использовать примеры из промпта как реальные данные")
            appendLine()
            appendLine("Покажи дашборд проекта с полной статистикой.")
            appendLine()
            appendLine("ОБЯЗАТЕЛЬНЫЙ Workflow:")
            appendLine("1. Вызови get_project_dashboard для получения метрик")
            appendLine("2. Вызови get_team_workload для анализа загрузки команды")
            appendLine("3. Опционально используй git_log для анализа недавней активности")
            appendLine("4. Сформируй комплексный отчёт на основе РЕАЛЬНЫХ данных:")
            appendLine("   - Общая статистика (всего задач, выполнено, в работе) из get_project_dashboard")
            appendLine("   - Распределение по статусам и приоритетам из get_project_dashboard")
            appendLine("   - Критичные и просроченные задачи (с КРАТКИМ описанием)")
            appendLine("   - Заблокированные задачи (с КРАТКИМ описанием)")
            appendLine("   - Загрузка команды из get_team_workload (РЕАЛЬНЫЕ имена исполнителей)")
            appendLine("   - Velocity (задач в неделю) и прогноз завершения из get_project_dashboard")
            appendLine("   - Недавняя Git активность (если релевантно)")
            appendLine()
            appendLine("При показе списков задач используй краткий формат:")
            appendLine("**<реальный_id>: <реальное_название>** | <статус> | <приоритет> | <краткое_описание>")
            appendLine()
            appendLine("Представь информацию визуально структурированно с эмодзи, используя ТОЛЬКО РЕАЛЬНЫЕ данные.")
        }
    }

    private fun buildTaskPrioritiesPrompt(): String {
        return buildString {
            appendLine("Проанализируй приоритеты задач и дай рекомендации.")
            appendLine()
            appendLine("Шаги:")
            appendLine("1. Вызови analyze_priorities для AI-анализа")
            appendLine("2. Вызови get_project_dashboard для контекста")
            appendLine("3. Используй git_log для анализа активности по задачам")
            appendLine("4. Предоставь:")
            appendLine("   - Общий анализ ситуации с приоритетами")
            appendLine("   - Рекомендации по изменению приоритетов")
            appendLine("   - Оптимальный порядок выполнения задач (с кратким описанием)")
            appendLine("   - Риски и способы их снижения")
            appendLine("   - Инсайты для улучшения планирования")
            appendLine()
            appendLine("При перечислении задач используй краткий формат:")
            appendLine("**ID: Название** | Приоритет | Краткое описание (до 100 символов)")
            appendLine()
            appendLine("Объясни логику рекомендаций понятным языком.")
        }
    }

    private fun buildTaskDetailsPrompt(taskId: String): String {
        return buildString {
            appendLine("Покажи детальную информацию о задаче $taskId.")
            appendLine()
            appendLine("Шаги:")
            appendLine("1. Вызови get_task для получения деталей")
            appendLine("2. Если у задачи есть git_branch или git_commits:")
            appendLine("   - Используй git_log для истории коммитов")
            appendLine("   - Используй git_diff для анализа изменений")
            appendLine("3. Покажи детальную карточку задачи:")
            appendLine("   - Заголовок: **ID: Название задачи**")
            appendLine("   - Полное описание задачи (все строки)")
            appendLine("   - 📊 Статус | 🔥 Приоритет | 👤 Исполнитель")
            appendLine("   - 🏷️ Метки | 📅 Срок | ⏱️ Оценка: estimated vs spent часов")
            appendLine("   - 🔀 Git ветка и коммиты (если есть)")
            appendLine("   - 📅 Создана: created_at | Обновлена: updated_at")
            appendLine()
            appendLine("При показе деталей задачи используй ПОЛНОЕ описание, а не краткое.")
        }
    }

    private fun buildTaskQueryPrompt(query: String): String {
        return buildString {
            appendLine("Ответь на вопрос о задачах проекта: $query")
            appendLine()
            appendLine("Используй доступные инструменты для получения актуальной информации:")
            appendLine("- list_tasks - для поиска задач")
            appendLine("- get_project_dashboard - для общей статистики")
            appendLine("- get_team_workload - для информации о загрузке")
            appendLine("- analyze_priorities - для анализа приоритетов")
            appendLine("- git_log - для анализа Git активности")
            appendLine()
            appendLine("При показе списков задач используй краткий формат:")
            appendLine("**ID: Название** | Статус | Приоритет | Краткое описание (до 100 символов)")
            appendLine()
            appendLine("Предоставь конкретный и полезный ответ на основе реальных данных.")
        }
    }

    private fun buildTaskHelpText(): String {
        return buildString {
            appendLine("**Team Assistant — доступные подкоманды:**")
            appendLine()
            appendLine("| Команда | Описание |")
            appendLine("|---------|----------|")
            appendLine("| `/task help` | Показать эту справку |")
            appendLine("| `/task create <описание>` | Создать задачу с AI-анализом приоритета |")
            appendLine("| `/task list [фильтры]` | Список задач (опционально с фильтрами) |")
            appendLine("| `/task update <id> <поле>=<значение>` | Обновить поля задачи |")
            appendLine("| `/task status` | Дашборд проекта с метриками и статистикой |")
            appendLine("| `/task priorities` | AI-анализ приоритетов и рекомендации |")
            appendLine("| `/task <task_id>` | Детали конкретной задачи |")
            appendLine("| `/task <вопрос>` | Произвольный вопрос о задачах проекта |")
            appendLine()
            appendLine("**Фильтры для `/task list`:**")
            appendLine("`status=open|in_progress|review|done|blocked|cancelled`")
            appendLine("`priority=low|medium|high|critical`")
            appendLine("`assignee=<имя>`  `label=<метка>`")
            appendLine()
            appendLine("**Поля для `/task update`:**")
            appendLine("`status`  `priority`  `assignee`  `title`  `description`")
            appendLine("`labels`  `due_date`  `estimated_hours`  `spent_hours`")
            appendLine()
            appendLine("**Примеры:**")
            appendLine("```")
            appendLine("/task create Добавить поддержку темной темы в UI")
            appendLine("/task list status=open priority=high")
            appendLine("/task update feature_005 status=in_progress")
            appendLine("/task update bug_001 status=done spent_hours=5")
            appendLine("/task status")
            appendLine("/task priorities")
            appendLine("/task feature_005")
            appendLine("/task Какие задачи заблокированы?")
            appendLine("```")
        }.trimEnd()
    }

    private fun buildTaskHelpPrompt(): String {
        return buildString {
            appendLine("Покажи краткую справку по команде /task.")
            appendLine()
            appendLine("Подскажи пользователю, что для полного списка аргументов можно ввести /task help.")
        }
    }
}
