...
fun AgentScreen(
    ...
    Column(
        ...
        Row(
            ...
            // Title with model, tools count and RAG indicator
            Text(
                text = buildString {
                    append("AI Agent")
                    append(" · ${settings.model.displayName}")
                    if (!settings.simpleChatMode && state.availableTools.isNotEmpty()) {
                        append(" · ${state.availableTools.size} tools")
                    }
                    if (!settings.simpleChatMode && settings.ragMode == RagMode.ON) {
                        append(" · RAG")
                    }
                },
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(4.dp)) {
                ...
                // New chat
                IconButton(
                    onClick = { store.dispatch(AgentIntent.NewChat) },
