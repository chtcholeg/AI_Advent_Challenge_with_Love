...
class VectorStoreImpl(
    ...
    override suspend fun search(
        ...
        val storedDim = chunks[0].embedding.size
        if (storedDim > 0 && queryEmbedding.size != storedDim) {
            throw IllegalArgumentException(embeddingDimensionError(storedDim, queryEmbedding.size))
        }

        return chunks
            .map { chunk ->
                val similarity = VectorMath.dotProduct(queryEmbedding, chunk.embedding)
    ...
    override suspend fun save(filePath: String) {
        ...
        val documentMap = chunks.groupBy { it.metadata.source }

        val indexedDocuments = documentMap.map { (source, docChunks) ->
            val sourceType = docChunks.firstOrNull()?.metadata?.sourceType ?: ru.chtcholeg.shared.domain.model.SourceType.FILE
            IndexedDocument(
                source = source,
    ...
    override suspend fun getStats(): IndexStats {
        val uniqueDocuments = chunks.map { it.metadata.source }.distinct().size
