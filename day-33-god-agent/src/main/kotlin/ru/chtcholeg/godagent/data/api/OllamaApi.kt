...
class OllamaApi(private val httpClient: HttpClient) {
    ...
    suspend fun getAvailableModels(baseUrl: String = DEFAULT_BASE_URL): List<String> {
        return try {
            val url = baseUrl.trimEnd('/')
            val response: OllamaTagsResponse = httpClient.get("$url/api/tags").body()
    ...
    suspend fun isAvailable(baseUrl: String = DEFAULT_BASE_URL): Boolean {
        return try {
            val url = baseUrl.trimEnd('/')
            httpClient.get("$url/api/tags")
            true
        } catch (e: Exception) {
            false
        }
    ...
    suspend fun sendMessage(
        ...
        val timedResult = measureTimedValue {
            val responseText = httpClient.post("$url/api/chat") {
