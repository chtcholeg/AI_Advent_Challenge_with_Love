...
class IndexerStore(
    ...
    private val _state = MutableStateFlow(IndexerState())
    ...
    private fun handleStartIndexing() {
        ...
        if (!_state.value.isOllamaAvailable) {
        ...
        indexingJob = scope.launch {
            ...
            indexerService.indexDocuments(path)
                .catch { e ->
                    _state.update {
                        it.copy(
                            isIndexing = false,
    ...
    private fun handleCheckOllamaStatus() {
        scope.launch {
            ...
            try {
                val isAvailable = indexerService.isOllamaAvailable()
                _state.update {
                    it.copy(
