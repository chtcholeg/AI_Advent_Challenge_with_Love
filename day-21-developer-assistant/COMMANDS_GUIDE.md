...
- 🏗️ Project Structure - структура модулей и директорий проекта
...
**Платформы:**
- **Desktop**: Читает полное содержимое README.md из корня Git репозитория, включая структуру модулей
- **Android**: Показывает статическое сообщение с основной информацией (без структуры проекта)

**Стратегия поиска README.md (Desktop):**
1. **Git repository root** (.git folder) - наивысший приоритет, всегда использует корневой README.md
2. **Gradle project root** (settings.gradle.kts) - если Git root не найден
3. **Directory hierarchy** - поиск вверх по дереву (до 5 уровней) - фоллбэк
