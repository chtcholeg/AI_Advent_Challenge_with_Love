...
class TaskCreateTool(
    ...
    override val description = """
        Creates a structured task list for your current coding session.

        Use this tool proactively when:
        - Complex multi-step tasks (3+ steps required)
    ...
    override val inputSchema = buildJsonObject {
        ...
        putJsonObject("properties") {
            putJsonObject("subject") {
                put("type", "string")
                put("description", "Brief, actionable title in imperative form (e.g., 'Fix authentication bug')")
            }
            putJsonObject("description") {
                put("type", "string")
                put("description", "Detailed description with context and acceptance criteria")
            }
            putJsonObject("activeForm") {
                put("type", "string")
                put("description", "Present continuous form for spinner (e.g., 'Fixing authentication bug')")
            }
            putJsonObject("metadata") {
                put("type", "object")
                put("description", "Optional arbitrary metadata to attach to the task")
                putJsonObject("properties") {}
