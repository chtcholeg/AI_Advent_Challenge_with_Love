    ...
    override val inputSchema = buildJsonObject {
        ...
        putJsonObject("properties") {
            putJsonObject("file_path") {
                put("type", "string")
                put("description", "Absolute path to the file to modify")
            }
            putJsonObject("old_string") {
                put("type", "string")
                put("description", "Exact text to replace (must match exactly as it appears in the file)")
            }
            putJsonObject("new_string") {
                put("type", "string")
                put("description", "Text to replace it with (must be different from old_string)")
            }
            putJsonObject("replace_all") {
                put("type", "boolean")
                put("description", "Replace all occurrences (default: false, only replaces if unique)")
                put("default", false)
            }
        }
