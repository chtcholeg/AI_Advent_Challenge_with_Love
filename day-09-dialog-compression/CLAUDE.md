...
**New Feature (day-09)**: Dialog Compression:
...
- **Compression Mode**: Always replaces history with summary to reduce token usage (default behavior)
- **Configurable Threshold**: Message threshold configurable from 4 to 50 messages (default: 8)
- **New Icon**: Create icon (Icons.Filled.Create - pencil/edit icon) for compression button - represents "creating a summary"
- **Setting**: `summarizationReplaceHistory` always true (compression mode is the only mode)
- **UI Change**: New Chat button (+) removed from main screen toolbar - accessible via Chat History screen
