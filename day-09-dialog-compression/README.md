# GigaChat Multiplatform Chat Application (Day 9 - Dialog Compression)
...
## New in Day 9: Dialog Compression

This version introduces **Dialog Compression** - a feature that automatically compresses long conversations by replacing history with a summary, reducing token usage while preserving context.

### How Dialog Compression Works
1. **Automatic Compression** (Optional): Enable **Auto-Summarization** in Settings and set the **Message Threshold** (default: 8 messages, configurable 4-50). When reached, the conversation is automatically compressed.
2. **Manual Compression**: Click the create/edit button (📝) in the top toolbar to compress the conversation at any time.
3. When compression occurs: history is **replaced** with a summary, reducing token usage while preserving context.
4. The AI continues the conversation based on the summary context.

### Benefits of Compression
- **Token Savings**: Reduces API costs by using summary instead of full history
- **Context Preservation**: AI continues conversation based on summary context
- **Automatic or Manual**: Choose between automatic compression after threshold or manual compression on demand
- **Always Enabled**: Compression mode is now the default behavior - no need to toggle settings

### Test Scenarios
See `COMPRESSION_TEST_SCENARIOS.md` for detailed test cases demonstrating good and bad compression scenarios.
...
- **Dialog Compression** (NEW in Day 9):
  - Automatic conversation compression when message threshold is reached
  - Manual compression via create/edit button (📝) in toolbar
  - Always replaces history with summary (compression mode)
  - Configurable message threshold (4-50 messages)
...
- **DIFF_08-09.md** - Detailed changelog between Day 8 and Day 9
