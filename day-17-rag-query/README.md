# GigaChat Multiplatform Chat Application (Day 17 - RAG Query)
...
## Updates in Day 17: RAG Query (Retrieval-Augmented Generation)

This version introduces **RAG Query** - the AI Agent can now use indexed documents as context for answering questions. When RAG mode is enabled, the agent automatically searches the vector index for relevant document chunks and injects them into the conversation context.

### New Features

#### RAG Pipeline in AI Agent
...
#### Document Indexer Application (New Module)
...
```
User Query → AgentStore → RagRepository → [EmbeddingService, VectorStore]
                                                      ↓
                                          Search index → Top-K chunks
                                                      ↓
                                          Format context → Inject into prompt
                                                      ↓
                                          GigaChat API (with RAG context)
```

### Document Indexing Pipeline

```
Documents → DocumentLoader → TextChunker → EmbeddingService → VectorStore → SQLite/JSON
                                                ↓
                                          Ollama / GigaChat API
```
...
# 3. Run the AI Agent with RAG
./gradlew :ai-agent:run
# Then: Settings → RAG Mode → ON, set index path
```
...
#### RAG Settings (AI Agent)

- **RAG Mode**: OFF / ON
- **Index Path**: Path to JSON index file
- **Top-K**: Number of relevant chunks to retrieve (default: 5)
- **Threshold**: Minimum cosine similarity score (default: 0.3)
...
│
├── indexer/                     # Document Indexer GUI (NEW in Day 17)
│   └── src/
│       ├── commonMain/
...
```bash
cd /Users/shchepilov/AndroidStudioProjects/AI_Advent_Challenge_with_Love/day-17-rag-query
```
...
- **Document Indexer GUI** (NEW in Day 17):
...
- https://disk.yandex.ru/i/nxG93ODu5icMzA
