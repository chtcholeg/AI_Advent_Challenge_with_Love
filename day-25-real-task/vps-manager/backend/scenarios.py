...
OPENVPN_STEPS = [
    {
        "id": "open_web_ports",
        "name": "Веб-порты (HTTP/HTTPS)",
        "description": "Открыть 80/443 если на сервере уже есть веб-приложение. Пропустите, если нет.",
        "type": "exec",
        "skippable": True,
        "inputs": [
            {
                "key": "open_web",
                "label": "Открыть HTTP (80) и HTTPS (443) для веб-приложения?",
                "type": "select",
                "options": ["нет", "да"],
                "default": "нет",
            },
            {
                "key": "web_port",
                "label": "Дополнительный порт приложения (необязательно, например 8001)",
                "type": "text",
                "placeholder": "8001",
                "default": "",
                "optional": True,
  ...
  echo '→ HTTP/HTTPS пропущены'
fi""",
            """if [ -n '{web_port}' ]; then
  ufw allow {web_port}/tcp comment 'Web App'
  echo '→ Порт {web_port} открыт'
fi""",
            "ufw status verbose",
        ],
    },
